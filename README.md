...
- 条件目标选择：钥匙弹窗按世界 / 当前二级地图切换，仅按名称过滤，有奖励来源时显示地图、节点类型和名称；节点通过现有画布拾取，可切换地图和楼层，点击节点旁的勾确认，Esc 取消，结束后恢复原地图、楼层、视角和选择。拾取期间暂停修改地图。
...
- 顶部固定的“钥匙管理”入口管理全局和当前二级地图的局部钥匙。局部钥匙只能在二级地图中创建，节点条件和奖励聚合全局与本地图的钥匙；预览调试切换持有钥匙；支持重置探索。
